...
public class globalAnnotatorUtils {
	...
	List<String[]> listOnto = globalAnnotatorUtils.getBioPortalOntologiesWithIds(group);
	...
	for (JsonObject result : results.getValuesAs(JsonObject.class)) {
		...
		for(int i = 0; i < arr.size();i++){
		    if(arr.getJsonObject(i).getString("acronym").equals(group)) ontologies.add(new String[] { result.getString("name"), result.getString("@id").substring(result.getString("@id").lastIndexOf('/') + 1) });			   
		}
	    }
	}
